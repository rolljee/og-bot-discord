...
function getCommerceMessage(message) {
  ...
  if (resource === "M") {
    ...
    const message = `[${rate}] ${prettify(number)} M contre ${
      prettify(response.crystal)
    } C ${prettify(response.deut)} D`;
    return message;
  } else if (resource === "C") {
    ...
    const message = `[${rate}] ${prettify(number)} C contre ${
      prettify(response.metal)
    } M ${prettify(response.deut)} D`;
    return message;
  } else if (resource === "D") {
    ...
    const message = `[${rate}] ${prettify(number)} D contre ${
      prettify(response.metal)
    } M ${prettify(response.crystal)} C`;
    return message;
  }
}
