...
function getCommerceMessage(message) {
	...
	if (resource === 'M') {
		...
		const response = Ogame.default.Trader.sellMetal(number, percentD, percentC, rate);
		...
		const response = Ogame.default.Trader.sellCrystal(number, percentD, percentM, rate);
		...
		const response = Ogame.default.Trader.sellDeut(number, percentM, percentC, rate);
